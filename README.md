## 👋 Hey, I'm Vladimir Pankratov

### 🧠 About Me
- 💼 Senior Software Engineer building open-source tools & plugins  
- 🦀 Rust, C++, Swift, Kotlin & cross-platform development  
- ❤️ Passionate about Tauri plugins, tooling & developer experience

---

## 🔧 Featured Projects

## 📱 Apps & Projects
| Project | Description |
| ------- | ----------- |
| [**PlanLyst**](https://github.com/Choochmeque/planlyst.app) | Encrypted cross-platform productivity planner |

### 🚀 Tauri Plugins
| Project | Description |
| ------- | ----------- |
| [**tauri-plugin-biometry**](https://github.com/Choochmeque/tauri-plugin-biometry) | Biometric auth & secure storage |
| [**tauri-plugin-google-auth**](https://github.com/Choochmeque/tauri-plugin-google-auth) | Google OAuth for Tauri |
| [**tauri-plugin-iap**](https://github.com/Choochmeque/tauri-plugin-iap) | In-App Purchases support |
| [**tauri-plugin-notifications**](https://github.com/Choochmeque/tauri-plugin-notifications) | Cross-platform notifications |
| [**tauri-plugin-sharekit**](https://github.com/Choochmeque/tauri-plugin-sharekit) | Native share interfaces |

### 📦 Tools & Libraries
| Project | Description |
| ------- | ----------- |
| [**tauri-macos-xcode**](https://github.com/Choochmeque/tauri-macos-xcode) | Generate Xcode projects for macOS Tauri apps | 
| [**tauri-apple-extensions**](https://github.com/Choochmeque/tauri-apple-extensions) | iOS extensions support for Tauri apps | 
| [**diesel_turso**](https://github.com/Choochmeque/diesel_turso) | Diesel ORM backend & async connection for Turso Database |
| [**msixbundle-rs**](https://github.com/Choochmeque/msixbundle-rs) | Windows MSIX packaging using Rust |
| [**SimpleGoogleSignIn**](https://github.com/Choochmeque/SimpleGoogleSignIn) | Lightweight iOS Google Sign-In |

---

## 🧠 Skills & Technologies
**Languages:** Rust 🦀, C++, Swift 🍎, Kotlin 🤖, TypeScript, JS  
**Platforms:** Android, iOS, macOS, Windows  
**Focus:** Security, Cross-platform tooling

---

## 📬 Where to Find Me

**LinkedIn:** https://www.linkedin.com/in/vlpankratov  
**Discord:** @Choochmeque  
*(Feel free to reach out)*

---
